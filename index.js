  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith("ticket_open_modal:")) {
        ...
        const otherUserId = interaction.fields.getTextInputValue("other_trader_id").replace(/[<@!>]/g, "").trim();
    ...
    if (interaction.isButton()) {
      if (interaction.customId === "open_ticket") {
        ...
        const modal = new ModalBuilder()
          .setCustomId(`ticket_open_modal:${interaction.message.id}`)
          .setTitle("Request Middleman");

        const other = new TextInputBuilder()
          .setCustomId("other_trader_id")
          .setLabel("Who is the other trader?")
          .setPlaceholder("Paste user ID or mention, e.g. 123456789012345678")
          .setStyle(TextInputStyle.Short)
          .setRequired(true);
